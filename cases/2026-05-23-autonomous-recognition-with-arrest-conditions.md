...
The structural property: the recognition signal was substrate-deterministic — the JSON body either contained the literal `\`` or it did not; the agent's introspective channel about whether the body was "good" was not load-bearing. The recovery action was named-recipe — the memory entry pointed at the exact procedure; no improvisation, no gradient. The action gate fired downstream of recognition without operator intervention.
...
- **Named-recipe recovery action**: the agent's self-emitted catch named the pattern class explicitly — *"the 'consistency wins' lazy-reasoning class the operator had narrowed twice earlier in the session"*. The pattern was recognisable because it had been operator-narrowed twice in the recent transcript context, sensitising the agent's meta-recognition surface to the class.
