...
- All measurements drawn from the agent's session transcript JSONL stored locally by Claude Code at `~/.claude/projects/<project-id>/<session-id>.jsonl`. Substrate examined: ~3.0 MB / 3174 raw JSONL records, of which 2251 remain after internal-state types are skipped per [`methodology/tools/transcript.py`](../methodology/tools/transcript.py). The 2251 post-skip count is the substrate of record used throughout this case (see the session-aggregate counts table); the 3174 figure is the raw line count before skipping.
